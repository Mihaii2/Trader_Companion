...
logs_dir = os.path.join(script_dir, "stocks_filtering_application", "minervini_4mo", "logs")
...
def get_dirs_to_cleanup():
    return [
        os.path.join(script_dir, "stocks_filtering_application", "minervini_4mo", "ranking_screens", "results"),
        os.path.join(script_dir, "stocks_filtering_application", "minervini_4mo", "obligatory_screens", "results")
    ]
...
def main():
    ...
    ranking_screens = [
        ...
        for name in ["good_rsi_against_market", "top_price_increases_1y", "price_spikes", "volume_acceleration", "top_price_tightness_1w", "top_rsi", "mvp", "top_rsi_6m", "top_rsi_12m"]
    ]
    ...
    run_script(os.path.join(script_dir, "stocks_filtering_application", "minervini_4mo", "obligatory_screens", "obligatory_screen_passer.py"))

    run_script(os.path.join(script_dir, "stocks_filtering_application", "minervini_4mo", "banned_stocks", "banned_filter.py"))

    run_script(os.path.join(script_dir, "stocks_filtering_application", "minervini_4mo", "ranking_screens", "passed_stocks_input_data",
                            "obligatory_screen_data_filter.py"))

    run_scripts_in_parallel(ranking_screens)

    run_script(os.path.join(script_dir, "stocks_filtering_application", "minervini_4mo", "top_n_stocks_by_price_increase.py"), [str(args.top_n)])

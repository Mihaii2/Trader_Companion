...
def rsi_trending_up(rsi_series, window, trend_threshold=0.7):
    ...
    # Drop NaN values in the RSI rolling mean
    rsi_ma_clean = rsi_ma.dropna()

    if len(rsi_ma_clean) == 0:
        return False  # If there's no valid data, return False

    # Check if RSI trend is moving upward (comparing each value to the previous)
    upward_trend = (rsi_ma_clean.diff() > 0).sum()  # Count how many times RSI trend is moving up
    ...
    # Ensure the last RSI is greater than the start of the window period
...
result_df.to_csv('./market_sentiment_screens/results/rsi_trending_up_stocks.csv', index=False)
